      ...
      _createClass(h, [
        ...
        },
        {
          ...
          value: function init(j, k) {
            k &&
              'object' ===
                ('undefined' == typeof k ? 'undefined' : _typeof(k)) &&
              this.extend(this.set, k)
            var m = document.createElement('div'),
              p = document.createElement('div'),
              q = this,
              r = q.set
            if (
              (m.classList.add('c_alert_dialog'),
              r.index && (m.dataset.index = r.index),
              p.classList.add('c_alert_wrap'),
              (p.innerHTML =
                '<div class="c_alert_con" style="' +
                r.style +
                '">' +
                j +
                '</div>'),
              r.addClass && p.classList.add(r.addClass),
              r.title &&
                (p.classList.add('c_alert_width'),
                p.insertAdjacentHTML(
                  'afterbegin',
                  '<div class="c_alert_title">' + k.title + '</div>'
                )),
              r.button)
            ) {
              p.classList.add('c_alert_width')
              var s = ''
              for (var u in r.button)
                s +=
                  '<a href="javascript:;" data-name="' + u + '">' + u + '</a>'
              p.insertAdjacentHTML(
                'beforeend',
                '<div class="c_alert_btn">' + s + '</div>'
              )
            ...
            r.time &&
              ...
              r.before && r.before.call(p),
              (void 0 === r.mask || r.mask) &&
              ...
              (void 0 === r.mask || r.mask) &&
                (m.querySelector('.c_alert_mask').onclick = function (w) {
                  w.preventDefault(),
                    (r.maskClick || void 0 === r.maskClick) &&
                      _D_obj.close(p, r.after)
                }),
              r.onload && r.onload.call(p),
              setTimeout(function () {
    ...
    (window._D_obj = {
      ...
      close: function close(h, j) {
        ...
        ;[].forEach.call(k, function (l) {
          ;(l.dataset.index == h || l === h.parentNode) &&
            (l.classList.remove('dialog_open'),
            l.classList.add('dialog_close'),
            j && j.call(l.querySelector('.c_alert_wrap'), h),
            l
              .querySelector('.c_alert_wrap')
